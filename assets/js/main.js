...
const text = {
  ...
  js:"JavaScript é uma linguagem de programação interpretada estruturada, de script em alto nível com tipagem dinâmica fraca e multiparadigma. <br><br> 1 ano de experiência",
  sass:"Sass (o pré-processador) é uma linguagem de folhas de estilo. É uma simples linguagem de script usada em arquivos Sass. <br><br> 9 meses de experiência",
  react: "React é uma biblioteca JavaScript de código aberto com foco em criar interfaces de usuário em páginas web. <br><br> 4 meses de experiência",
...
function changeArticleText() {
  knowning.forEach(function(el){
    el.addEventListener("mouseover", function () {
      ...
      :(el.classList.contains('sass'))? text1.innerHTML = text.sass 
      :(el.classList.contains('react')) ? text1.innerHTML = text.react 
      :(el.classList.contains('next')) ? text1.innerHTML = text.next 
      :(el.classList.contains('styled')) ? text1.innerHTML = `Em Processo..`
      :(el.classList.contains('tailwind')) ? text1.innerHTML = `Em Processo..`
      : ''
  })
 })
}
...
var leftBox = document.querySelectorAll(".sairCaixa");
var text1 = document.querySelector(".mudarTextoDescricao");
var leftBox = document.querySelectorAll(".sairCaixa");

for (let element of leftBox) {
  element.addEventListener("mouseout", ()=>  text1.innerHTML = `*passe o mouse no card para ler*`)
}  
...
/* for (i = 0; i < sairCaixa.length; i++) {
  sairCaixa[i].addEventListSener("mouseout", function mudarTextoNormal() {
    text1.innerHTML = `*passe o mouse no card para ler*`;
  });
}  */
